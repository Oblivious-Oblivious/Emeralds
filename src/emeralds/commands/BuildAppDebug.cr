class Emeralds::BuildAppDebug < Emeralds::Command
  ...
  def block
    -> {
      ...
      make_export;
      TerminalHandler.generic_cmd "#{OPT["cc"]} #{OPT["debug_opt"]} #{OPT["debug_version"]} #{OPT["debug_flags"]} #{OPT["debug_warnings"]} #{OPT["unused_warnings"]} -o #{OPT["output"]} $(#{OPT["input"]}) $(#{OPT["inputfiles"]}) $(#{OPT["deps"]})", display: true;
